	...
	$.widget("ui.coverflow", {
	   ...
	   options: {
		...
		_create: function() {
			...
			/* 
			=============================================== 
			The multiplier below seems to require changing 
			whenever the total number of pedals changes 
			(usually each month)

			this.itemSize = (CHANGE_THIS_NUMBER * this.items.length) * this.items.innerWidth();
			...
			*/
			...
			this.itemSize = (0.01275 * this.items.length) * this.items.innerWidth();
		...
		select: function(item, noPropagation) {
			...
			var to = Math.abs(self.previous-self.current) <=1 ? self.previous : self.current+(self.previous < self.current ? -1 : 1);
		...
		_refresh: function(state,from,to) {
			...
			this.items.each(function(i) {
				...
				if(vendorPrefix == 'ms' || vendorPrefix == ""){
					...
					css.width = self.itemWidth * (0.65+((1-mod)*0.35));
					css.height = css.width * (self.itemHeight / self.itemWidth);
					css.top = -((css.height - self.itemHeight) / 3);

					css.left -= self.itemWidth/6 -36;
					}
					else{
					css.width = self.itemWidth * 0.75;
					css.height = css.width * (self.itemHeight / self.itemWidth);
					css.top = 28;
