...
xml.product name: @product.name do
  xml.link url: product_url(@product, host: @product.brand.default_website.url)
  xml.id @product.friendly_id, type: 'text'
  xml.brand @product.brand.name, api_url: api_v2_brand_url(@product.brand, host: @product.brand.default_website.url).gsub!(/\?.*$/, '')
  ...
  xml.downloads do
    @product.active_softwares.each do |software|
      xml.item(
        ...
        url: software_url(software, host: @product.brand.default_website.url),
