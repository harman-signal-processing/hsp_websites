module ProductsHelper
  ...
  def parse_tabs(tabs, product)
    ...
    r << ProductTab.new("documentation") if (product.product_documents.size > 0 || product.site_elements.size > 0) && selected_tabs.include?("documentation")
    r << ProductTab.new("training_modules") if product.training_modules.size > 0 && selected_tabs.include?("training_modules")
    r << ProductTab.new("downloads_and_docs") if (product.softwares.size > 0 || product.product_documents.size > 0) && selected_tabs.include?("downloads_and_docs")
